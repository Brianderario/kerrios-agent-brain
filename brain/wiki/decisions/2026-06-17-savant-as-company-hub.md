...
**Phase 5 — demote git to a pure backing store: pending.** With reads, knowledge authoring, and the operating layer all flowing through Savant, Phase 5 formalizes git as the silent versioned backing store + the runner's on-disk copy. Ships and verifies independently; can run interactively or as a build-loop task list ([[../../../agent-prompts/build-loop/SKILL.md]]).
