...
- **No compliance footer at all (Brian, 2026-05-30).** Brian's call, in two steps: (1) postal address dropped as a blocker, then (2) the whole footer removed — no unsubscribe line, no address. Rationale: these are 1:1 cold emails, not a newsletter/sequence; they should read like a normal personal note. It was a restriction Kerri introduced unprompted, not an existing org rule. Opt-outs are STILL honored — the inbox-sweep auto-suppression (STEP 2b) detects "unsubscribe / remove me / stop" from a reply body and hard NDR bounces, and adds them to `cold-do-not-contact.json`, independent of whether we ever invited an opt-out. If Brian ever wants formal CAN-SPAM compliance back, re-add the footer in cold-outreach SKILL STEP 4 + HARD RULE 8b.
