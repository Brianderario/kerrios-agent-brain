---
slug: simplexity
company: "Simplexity Product Development"
jobId: H0127
prefix: H
domain: simplexitypd.com
status: active
relationship_tier: renewal
primary_contact_name: "Michael Tanguay"
primary_contact_email: michael.tanguay@simplexitypd.com
secondary_contacts: []
mailbox: brian@hardwarefyi.com
send_from: brian@hardwarefyi.com
last_contact_date: 2026-06-07
last_sender: us
last_message_subject: "RE: <EXTERNAL EMAIL> Re: Kinetic HW Conference"
thread_internet_message_ids: []
next_action_date: null
last_nudge_date: null
nudge_count: 0
created_at: 2026-06-09
updated_at: 2026-06-09
source: kinetic-reconnection
---

# Simplexity Product Development -- Active deal (newsletter sponsorship)

Product development consultancy. Brian followed up Jun 7 asking about newsletter sponsorship: "if you have a rough budget or timing in mind, send it over and I can put together a few options across The Analog / Weekend Wire." (Note: Brian's email used the old edition names; going forward, refer to it as the Hardware FYI newsletter, twice weekly Tue/Sat.)

## Pipeline state

- **Status: active.** Newsletter sponsorship follow-up sent Jun 7. Waiting for Michael's budget/timing.
- **Last sender: us** (Brian follow-up Jun 7).
- Benji CC'd.

## Source
- Post-Kinetic HW Conference conversation. Michael expressed interest in newsletter sponsorship earlier.
