...
Every `⚠️ PIPELINE UPDATE NEEDED` task whose resolution is "open a new deal" must carry that deal as a machine-executable `on_complete` payload (Brian rule 2026-06-12; the Console executes it server-side the moment Brian marks the card done, and skip never executes). File it with `scripts/console-task-api.mjs create ... --on-complete-json '{"action":"create_deal","params":{"name":"<deal name>","company_name":"<Company>","deal_type":"sponsorship","stage":"<lead|qualified|proposal_sent|contract_sent|negotiation>","value":<number-or-omit>,"property_slug":"<slug>","expected_close_date":"YYYY-MM-DD","notes":"<source pointer>"}}'`. Map `stage` from the signal table above and follow the dollar-value evidence rules below (omit `value` when pricing is unproven). The Console review page shows Brian exactly what marking done will trigger. A new-deal task filed without the payload forces Brian into manual CRM data entry and is a filing defect.

Dollar values require source-backed commercial terms. Brian confirmed the posture explicitly (2026-06-12, Jiga year-two): placeholder deal rows are WELCOME for any real prospect or active relationship — open them freely — but never with an invented price. Phantom dollars in the pipeline destroy his read on where the business actually stands; an unpriced placeholder row keeps the opportunity visible without lying about the number. If Brian/Kerri has touched the company but no pricing/proposal/counter/contract/invoice has been sent or received, keep the company in pipeline as `Prospect` or `Interest` with amount `TBD` / zero ledger value. Do not add estimated pricing from the target list, prior sponsor norms, or "likely close" analysis. Once a proposal/quote has actually been sent (stage `proposal_sent` or later), ALWAYS assign the Savant deal value — never leave a sent-proposal deal unpriced (Brian rule 2026-06-18): for a **menu of options**, book the **middle-of-the-road** option (the central recommendation, not the cheapest and not the top outlier); for a **specific recommended package**, book the **package price**. Examples (2026-06-18): Rev1 menu (Custom Article $5K / SF Tech Week $15K / $6K / Happy Hour $5K) → $5,000; GrayMatter recommended 1-month trial package → $5,000. Pass the number explicitly via `--value` to `scripts/console-pipeline-update.mjs`: the built-in middle-of-three auto-picker only fires when the evidence text has exactly three dollar figures, so menus with four+ options or a full rate card fall through to null, and the agent must supply the value (it is a judgment call, not a regex). This refines the no-phantom rule above rather than contradicting it: keep value `TBD` BEFORE a price is sent, but a sent menu/package is a real quote, so price the deal. The summary `Pipeline Amount` is priced open pipeline only; unpriced real opportunities stay visible but do not count toward the amount.
