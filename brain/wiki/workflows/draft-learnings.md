...
## [2026-06-09] Job H0059 — GrayMatter Robotics (second lesson on same job)
**What changed:** Brian flagged the queued reply to Obi (Rand's employer-brand intro) and asked for a rewrite against the actual chain. The original draft pitched a sponsored partnership and asked about "goals and budget" while ignoring Rand's 6/4 message ("we generally prioritize earned media over paid features... still open to reviewing details depending on scope and budget") from two messages earlier. The rewrite acknowledges the scope/budget caution directly and offers examples + pricing as a lower-friction step before the call.
**Why it happened:** The sweep drafted from the latest message plus its quoted tail. Gmail-style quoting only carried Kerri's 6/5 reply, not Rand's 6/4 earned-vs-paid message, so the budget sensitivity vanished from drafting context even though the task's WHAT'S GOING ON summary mentioned it. Summary altitude is not drafting altitude.
**Rule:** Before drafting ANY reply, call the thread tool and read the full chain oldest to newest; the quoted tail and prior job summaries do not count. Every still-live sensitivity from earlier in the chain (pricing posture, earned-vs-paid, budget caution, a prior "no") must shape the draft or be consciously dropped with a reason. Enforced structurally in kerri-inbox-sweep SKILL.md (FULL THREAD READ threadState artifact + fail-closed ACTION: redo, 2026-06-09).

## [2026-06-09] Job H0139 — beehiiv (skipped by Brian)
**What changed:** Brian checked the post-call vendor-onboarding acknowledgement task but set line-1 ACTION to `skip, let benji handle` rather than sending.
**Why (inferred):** beehiiv migration (newsletter platform move) is Benji-owned operations. The inbound shared resources with no hard ask, so a Brian-signed acknowledgement was unnecessary noise on a thread Benji already sits on.
**Rule:** For vendor/tooling onboarding follow-ups with no hard ask where the owning teammate (Benji for newsletter/ops) is already on the thread, prefer skip/no-draft over a courtesy acknowledgement. Surface it as a light flag, not a Brian-sender reply.

- 2026-06-11 (inbox-sweep, MFG Flow H0122): a pipeline-followup nudge draft must mirror what was ACTUALLY sent in the live thread, not the deal/CRM product-rec shorthand. The deal record said "webinar $5K + 6-month Partner Program $12.5K" but Brian's actual 6/5 email to Anshu offered webinar $5K / custom content (priced by format) / newsletter $1.5K each, with no Partner Program line. The auto-drafted nudge referenced "the two options I sent ... Partner Program $12.5K" which Anshu never received. Caught at STEP 2 before sending (P8 brain-log + live-thread reread). Rule: before approving/sending any "following up on what I sent" nudge, reread the sent item and quote only what was sent. pipeline-followup drafts also shipped with empty internetMessageIds (no route) — verify route before queueing.

- 2026-06-11 (inbox-sweep, Jiga H0034 placements redo): the Hardware FYI **newsletter placement/ad calendar is owned by Brian + Kerri as a sales asset** — Benji writes the newsletter content, he does NOT run the placement calendar. When a sponsor asks to move/confirm placement dates, Kerri has the open dates and should give the actual dates in the reply, not defer to Benji. The newsletter sends Tuesday + Saturday, so "move me to Saturdays" is satisfied by mapping each placement onto consecutive open Saturdays, and Saturdays are inherently clear of any Tuesday-only advertiser (here, Fictiv, Jiga's competitor). Rule: for placement-scheduling replies, pull/propose the real dates from the ad calendar yourself; only flag a date for Brian to verify when it is operationally tight (e.g. a 2-day-out issue) or co-runs with another booked sponsor. (Brian redo guidance, 2026-06-11.)
- 2026-06-11 (inbox-sweep, Plexus H0128 redo): reinforces the [2026-06-09 H0059] full-chain rule for REDOS specifically — a redo must re-read the live thread, not regenerate from the task summary. Brian sent back a generic "we missed each other, still want to talk?" Plexus draft because it ignored that Ann had booked then no-showed the 6/8 call and that the substance (digital + Kinetic) was never named. Rule: on any redo, read the full chain first; a warm re-engagement should name the missed touchpoint specifically (no blame) and give a concrete reason to re-engage, not a vague check-in.

- 2026-06-11 (inbox-sweep, Benji Westin-bill Q): when a teammate asks Kerri to explain a bill/invoice, OPEN the attached statement and explain the composition the first time, not just restate the summary totals. Kerri's first 22:37 reply repeated Westin's two account-number lines ($7,148.34 refund / $939.16 owed) without saying WHAT each was, so Benji came back asking "why do we owe an extra $939.16?". The answer was in the attached PDFs: acct 4751878 = Brian's 2-night guest-room folio (not a penalty/extra); acct 4716592 = $196,666.45 deposits prepaid vs $189,518.11 F&B = the overage being refunded. Rule: for an internal "explain this bill" ask, read the attachment and state what each charge/credit is for, so the follow-up question is pre-empted.

## [2026-06-12] Weekly edit-rate review — the two hand-edited sends of 6/06-6/12 (Brian-directed via iMessage)
**What changed:** Brian reviewed the week's 31 sends (2 hand-edited) and asked for the lessons to be committed. (1) MFG Flow H0122: Brian cut the body down to a short reply — the draft was too long for a warm contact with a simple ask. (2) Jiga H0034 year-two renewal: Brian's hand-edit added the Kinetic 2027/Fictiv line — relationship context that was already in the brain (Khay moved to Saturdays specifically to avoid Fictiv; Kinetic is the natural year-two upsell) but never made it into the draft.
**Why:** Both edits share one root cause: drafting from the thread instead of from everything the company knows. Length and substance both have to come from the relationship, not the latest message.
**Rule (two parts):**
1. **Relationship-context pull before drafting to any existing relationship:** load the CRM deal state plus cross-property context (open renewals, event involvement, competitor sensitivities, who owns the account) and weave what is relevant into the draft, or consciously drop it with a reason. The email chain alone is not the company's full context.
2. **Match length to the inbound on warm threads:** a three-sentence email gets a short reply, never a twelve-sentence one. Long bodies are reserved for genuinely new substance (proposals, multi-question answers), not warmth.
...
## [2026-06-21] Self-improve pattern — never ship a draft with a bracketed `[PASTE …]` placeholder token (job G0023)

**What changed:** On G0023 (post-call follow-up to Phil Strazzulla), the otherwise-finished body read "Kinetic … at `[PASTE KINETIC LINK]`, and Future Proof … at `[PASTE FUTURE PROOF LINK]`." Brian filled the real URLs (`https://kinetic.hardwarefyi.com/`, `https://futureproofhq.com/festival/`) before sending. Surfaced by the 2026-06-21 self-improve edit-pattern scan.
**Why:** This is the narrow, mechanically-detectable cousin of the empty-placeholder follow-up failure ([2026-06-17] above): the email was complete and on-voice, but a literal placeholder token survived into the approval-ready draft instead of being resolved at draft time. Both links were trivially knowable. A placeholder token that reaches the queue forces Brian to do the last-mile fill the draft was supposed to finish — the same last-mile miss as the Definition-of-Done gate, in its most avoidable form.
**Rule:** Never let a draft reach the approval queue containing a bracketed placeholder token — `[PASTE …]`, `[INSERT …]`, `[LINK]`, `[URL]`, `[TBD]`, `[XXX]`, `[NAME]`, `[DATE]`, or any all-caps `[BRACKETED]` stub. Resolve the real value before drafting (links, dates, figures are almost always knowable via tools/brain/Drive); if a fact is genuinely unknown, ask Brian for that one piece rather than shipping the placeholder. Proposed as a deterministic `presend-lint.mjs` check (see 2026-06-21 improvement page, proposal #1) so this becomes a hard mechanical block like the em-dash gate. Related: [[feedback_definition_of_done_gate]], [[presend_lint_gate]], and the [2026-06-17] post-call follow-up failure pattern above.

## [2026-06-25] SALES RULE — never auto-propose a specific package tier in a follow-up/nudge (Brian-flagged)

**What changed:** On the CoLab Software pipeline nudge #2 (deal H0030), the draft opened "the $5K newsletter-only tier is probably the cleanest start given the budget … Should I pencil in a July issue?" Brian requested a redo: "Rewrite this just following up; As a standing rule, we should never auto propose one of the packages, what if he wants to go for the higher one? Bad sales tactics."
**Why:** Pre-selecting the cheapest tier for the buyer anchors them down and forecloses a larger deal. The buyer should pick the level. A follow-up's job is to reopen the conversation and offer to find the right fit, not to choose for them. This is the nudge/follow-up cousin of [[feedback_no_price_negotiation]] (don't debate price) and [[feedback_sales_value_first]] (sell value, dollars are downstream).
**Rule:** In any follow-up or nudge to a warm prospect who already has options/pricing, do NOT name or recommend a specific package tier. Reopen the thread, ask where they stand, offer a quick call to land on the option that fits, and offer proof (placement click-throughs, audience fit). Let the buyer choose the level, including a higher one. Reshaping a package to a stated budget is still fine once they engage; pre-anchoring them on the cheapest tier unprompted is not. Mirrors Claude memory [[feedback_no_auto_package_tier]].

## [2026-06-28] Self-improve pattern — default to contractions; don't draft in stiff expanded register (H531, H0157, H0189)

**What changed:** Across three sends this period, Brian restored contractions Kerri had spelled out in full. H531 (Andustry nudge): `there is`→`there's`, `Let us`→`Let's`, `you are`→`you're`. H0157 (Renaud / Kinetic nudge): `that is easier`→`that's easier`. H0189 (Viam / Alyssa): `I will`→`I'll`. Surfaced by the 2026-06-28 self-improve edit-pattern scan.
**Why:** Kerri's default drafting register is one notch too formal — expanded forms read like a press release, not how Brian (or Kerri) actually talks. This is the concrete, checkable tell behind the existing "too clean / too sterile / butler-toned" voice note (voice.md, see line ~87 above): that note warns against sterilizing tone but never named the mechanical cause. It recurred across two action classes (pipeline-nudge, sponsor-substantive-reply) in one week, so it's a broad register rule, not a one-off.
**Rule:** In external prose and Brian-signed internal sends, default to contracted forms — `there's`, `here's`, `let's`, `you're`, `we're`, `we'll`, `I'll`, `I'm`, `it's`, `that's`, `don't`, `won't`, `can't`, `isn't`, `they're`. Use the spelled-out form only where the emphasis is deliberate (e.g. "I will not" for firmness). Before queueing any draft, scan for spelled-out contraction candidates and contract them. Related: voice.md anti-sterile note (line ~87), [[feedback_definition_of_done_gate]]. Proposed (lower-priority) as an optional `presend-lint.mjs` soft-warn in the 2026-06-28 improvement page, proposal #3 — but the primary fix is drafting-time.
