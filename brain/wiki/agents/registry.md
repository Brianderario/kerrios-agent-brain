...
| `kerri-revenue-standup` | 4pm ET Fridays; LIVE since 2026-06-11 as an **Anthropic cloud routine** (trig_01FPKyYXmY4goNNHdkdXLLSz), not a local task (the local Mac shim is disabled); read-only scoreboard, reports via Slack DM. See the 2026-06-11 cloud-routines note below. | `agent-prompts/kerri-revenue-standup/SKILL.md` |
...
2026-06-11 note (cloud-hosted routines): The first cloud routines went live on Anthropic infra (Claude Code cloud routines, no laptop required, no extra spend). **`kerri-revenue-standup` moved fully to the cloud** (trig_01FPKyYXmY4goNNHdkdXLLSz, Fridays ~4pm ET) and the local Mac scheduled task was disabled. It still EXISTS and runs, just on Anthropic infra, signed "Kerri (cloud)". Two additive cloud routines also run with no local equivalent: `kerri-morning-brief-fallback` (weekdays 7:25am ET) and `kerri-eod-meetings-recap` (weekdays 7:05pm ET). List/manage them with `claude -p "/schedule list"` (delete is dashboard-only at claude.ai/code/routines). IMPORTANT: cloud routines report via Slack DM, NOT via `create_agent_run`, so the Savant Console and the local liveness check cannot observe their runs. `kerri-revenue-standup` therefore carries no `report_interval_hours` and is intentionally excluded from the Console `AgentHealthCheckJob` overdue scope (otherwise it false-flags "Agent overdue" daily, which it did until 2026-06-23). It is active; do not mark it retired or paused, and do not re-add a report interval unless the cloud routine is wired to POST `create_agent_run`.
