...
3. **Dedup is absolute.** Skip anyone whose **company** is in the KMG Console CRM (`GET /api/v1/companies?domain=<d>`, which matches domain AND aliases; also fuzzy-name check; the read-only snapshot `data/companies.json` is the offline fallback) — existing customer/relationship, don't cold-prospect. Also skip anyone who is already a Savant contact (`GET /api/v1/people?company_id=<id>`), anyone in `data/jobs.json` history, `data/cold-do-not-contact.json`, `data/cold-outreach-state.json#sent` within 90 days, anyone already in `data/leads-master.json`, and anyone already in `data/cold-outreach-queue.json`. The Console CRM check is the **primary** dedup — see [[../../brain/wiki/workflows/customer-id-protocol]]. (`brain/wiki/people/` is frozen and is NOT a dedup source.)
   - **3a. MAILBOX dedup is MANDATORY — the CRM is NOT a record of who we've emailed (added 2026-06-19 after a queue audit found ~40% of the top-of-queue was already-contacted).** The single biggest source of wasted cold cycles is queuing companies we already reached. The CRM misses them because Brian's manual outreach (e.g. the Jan 2026 Kinetic push) and inbound contacts never got CRM rows. So, BEFORE queuing any candidate: (i) skip any domain present in `data/cold-contacted-domains.json` (the persistent cache of every domain we've ever cold-contacted or had two-way contact with — cheap, check first); and (ii) for any candidate that clears the cache, do a quick mailbox-history search of the HWFYI mailboxes (`mcp__brian-hardwarefyi-email__search_email`, `mcp__kerri-hardwarefyi-email__search_email`, `mcp__info-hardwarefyi-email__search_email`, inbox AND sentitems) plus Brian's kerrihq Gmail for the domain and contact name. ANY prior two-way contact → skip the lead, do NOT queue it, and append its domain to `data/cold-contacted-domains.json` so it's caught cheaply next time. The cache is the cheap first pass; the per-candidate mailbox search is the backstop for what the cache doesn't yet hold. (Cold-outreach also consults the same cache in its preflight, so a missed already-contacted lead burns its scarce draft budget — keep them out of the queue here.)
...
5b. **Multiple contacts at big companies (Brian rule, 2026-06-10).** For any company with ≥500 employees, one contact is not enough — the first pick is unlikely to be the right buyer. Capture 2-3 marketing/GTM contacts: the primary on the lead's `email`/`name`/`title` fields, the rest in `altContacts: [{name, title, email, linkedin}]`. Diversify titles across the set (e.g., one corporate/brand marketer + one demand-gen/field marketer + one partnerships/sponsorships person if one exists; "Events Marketing" and "Sponsorships" titles are gold for HWFYI). Under 500 employees, one contact is fine.
6. **Stable key, but NO premature customer ID.** Each pool lead is keyed by `leadId` = the company's lowercased root domain (e.g. `acmehw.com`). Do NOT assign a customer `jobId` or register the company in the KMG Console at discovery time — cold prospects are not yet customers, and registering hundreds would pollute the CRM of record and bump the H counter. The customer `jobId` is assigned later, only when the lead is actually contacted (cold-outreach runs the [[../../brain/wiki/workflows/customer-id-protocol]] at draft time) or replies; it backfills onto the pool lead then. You STILL dedup against the Console CRM by domain/alias/fuzzy-name (snapshot `data/companies.json` when offline) to avoid cold-prospecting an existing customer/relationship — read it, never write it.
    ...
    "leads": [
      {
        ...
        "altContacts": [],            // 2-3 extra marketing/GTM contacts for companies ≥500 employees (HARD RULE 5b): [{name, title, email, linkedin}]
...
- KMG Console CRM via `GET /api/v1/companies` (read-only snapshot `data/companies.json` when offline) — existing relationships (dedup + lookalike seeds)
- Savant CRM contacts (`GET /api/v1/people`) — existing contacts (dedup); `brain/wiki/people/` is frozen, not a dedup source
...
3. **Queue top-up (`data/cold-outreach-queue.json`).** Only AFTER the pool + CRM are written: take the highest-scored pool leads with `status: new` AND a concrete hook, prioritizing leads with a clear CY2026 revenue path from `hwfyi-cy2026-revenue-goal.md`, and append enough to bring the queue to **at least 25 entries** (don't exceed the queue's 100-entry cap; prune lowest-scored stale entries). Flip those leads' `status` to `queued` in the pool. Queue entry shape:
   - **Keep the ENTIRE queue sorted highest-score-first after every top-up (added 2026-06-19).** Cold-outreach inspects only a bounded slice from the TOP each morning (~25 entries), so a strong lead buried at position 95 never gets drafted. Do NOT just append new leads to the end — re-sort the whole queue by `score` descending (entries tagged `status: needs-qualification` sink to the bottom) so the best leads are always what cold-outreach sees first. A 2026-06-19 audit found $30-85M-funded robotics leads (score 40-50) sitting below generic score-15 software-adjacency entries, which is why batches came up short.
   - **Audience filter (added 2026-06-19).** Do NOT queue companies whose audience is not commercial hardware/electronics/manufacturing engineers: pharma/life-sciences/biotech/medtech validation, health advisory, AEC/architecture/construction, real-estate, logistics/supply-chain ops platforms, retail/fashion. They fail cold-outreach's high-intent bar and waste its draft budget. They can stay in the pool as `status: out-of-icp` but never enter the queue.
   ```
...
- Write to the KMG Console CRM (or `brain/wiki/companies/`, which is frozen) — only the discovery batches in `data/lead-research/` capture raw research. Companies get a Console record only after a real relationship starts (post-send or post-reply via inbox-sweep).
