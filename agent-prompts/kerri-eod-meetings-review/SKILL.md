...
- **ATTACHMENT DISCIPLINE — declare a canonical slug, never a link or a path.** To put a file on a send, call `list_canonical_attachments`, then declare `ATTACHMENTS TO SEND: <slug>` on the card (e.g. `ATTACHMENTS TO SEND: kinetic_prospectus`, or `... : kinetic_prospectus and media_kit`). The Render sender (`SendApprovedDraftsJob`) resolves the slug to the real, pre-verified bytes and attaches them — no Drive round-trip, no ids, no size surprises. NEVER paste a Google Drive/Docs link or a file path into the email body: the sender holds any client draft whose body contains a Drive link (`held_reason: attachments_unresolved`). For a per-client report, generate it with the document tools and declare `ATTACHMENTS TO SEND: <name>.pdf (Document ID: <id>)`. For a genuine one-off not in the registry, put it in Drive with a clean client-facing name, verify `fileSize` matches the source (a 6.7MB PDF once truncated to 251 bytes), and declare `... (Drive ID: <id>)` — then ask to add it as a canonical slug if it will recur. Limit: 3 files/email, ≤20MB each. Full contract: CLAUDE-ROUTINES.md → "Email attachments contract".
