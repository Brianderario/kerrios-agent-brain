...
These are DISCOVERY inputs, NOT a bypass of the bar: every item still needs a dollar/output figure, a named source, and an in-window publication date, verified against the underlying source URL. A tracked-company story beats a generic trade-press item of equal magnitude, and the Wire's cross-company patterns often carry the issue's Lead hook (feed them into STEP 4 too). Apply the scope filter (route only hardware / defense / energy / mfg / maritime / materials items; let AI / software / biotech pass to Hardware FYI). If Brian's Chrome or the Wire is unavailable at run time (this routine fires headless at 8pm ET), skip the live Wire/X pull and go straight to the publication scan (3b) — you can still use the static company roster in `relay-builders-directory.md` (path in `tracked-companies.json`) to boost any 3b story about a tracked company. The Wire is an enhancement, never a blocker on shipping the issue.
...
3. **Adversarial critic pass.** Spawn ONE sub-agent as a ruthless copy critic. Give it the drafted Lead, the playbook, and the **last 2–3 issues' Leads, their recorded frames, AND their headline syntactic templates** (from `data/sw-newsletter/state.json#recentTopics[*]` — the `leadFrame` and `leadHeadlineTemplate` fields), so it can catch both a recycled rhetorical frame AND a recycled headline construction. Instruct it to flag: every AI-writing tell (negation formula, constructed rule-of-three, setup-payoff filler, em dashes, hype, performing-insight phrases); every buried lede; every oversized sentence; any open that is not a real hook; and any headline whose syntactic template (e.g. "X Trades Y for Z", "X Becomes a Y Town") matches a template from any of the last 2–3 issues — a fresh topic on a recycled headline pattern still reads formulaic. It returns a ranked objection list; it does NOT rewrite.
...
5. **Record the frame and the headline template.** Once the Lead is final, append two fields to the matching `recentTopics` entry in `data/sw-newsletter/state.json`: (a) `leadFrame` — the one-line rhetorical move the Lead actually used (inversion / rule-first / naive-question / isomorphism-to-X / so-what-flip); and (b) `leadHeadlineTemplate` — the syntactic pattern of the headline as a short 4–7 word schema capturing grammatical structure, not specific nouns (e.g. "X Trades Y for Z", "X Becomes a Y Town", "X Finds Y in Z", "The X Nobody Was Y-ing"). Both give the next run's critic independent repetition signals: `leadFrame` catches a recycled rhetorical move on a fresh topic; `leadHeadlineTemplate` catches a recycled headline construction even when the rhetorical move differs.
