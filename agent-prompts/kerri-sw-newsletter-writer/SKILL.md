...
3. **Adversarial critic pass.** Spawn ONE sub-agent as a ruthless copy critic. Give it the drafted Lead, the playbook, and the **last 2–3 issues' Leads and their recorded frames** (from `data/sw-newsletter/state.json`, which logs each issue's Lead frame — see step 5), so it can catch a frame recycled from any recent issue, not only the immediately prior one. Instruct it to flag every AI-writing tell (negation formula, constructed rule-of-three, setup-payoff filler, em dashes, hype, performing-insight phrases), every buried lede, every oversized sentence, and to verify the open is a real hook. It returns a ranked objection list; it does NOT rewrite.
...
5. **Record the frame.** Once the Lead is final, append its one-line frame label — the rhetorical move it actually used (inversion / rule-first / naive-question / isomorphism-to-X / so-what-flip) — to `data/sw-newsletter/state.json` alongside the issue topic. This gives the next run's critic a rolling memory of recent frames to diff against: a fresh topic carried on a stale frame still reads as formulaic, and topic-dedup alone won't catch it.
