...
>
> We reach 17,000 hardware engineering leaders every week across two newsletters and create custom content with our partners to drive high value engagement from our readers.
>
...
- **Sponsors / prospects (cold or re-opening)** — slightly warmer opener ("Hi X, I hope you're doing well!"), credibility numbers, low-friction next step. **Default sender for ALL outreach (cold AND re-engagement) is Kerri from `kerri@hardwarefyi.com`** — set 2026-05-25 by Brian. Sender flips to Brian (`brian@hardwarefyi.com`) ONLY when the thread is genuinely founder-to-founder or Brian has already been the prior sender on the thread. **Kerri-as-sender role intro (exact phrasing, set 2026-05-25):** `"I'm Kerri, and I work on partnerships at Hardware FYI."` Note the deliberate softness — "work on" not "lead," and self-introduction by name first because the recipient does not know her. This is a peer-level, humble framing that role-matches the partnership ask without overclaiming. **Lead with value to the recipient**, never with what we're selling. No mention of "sponsor menu," "sponsor packages," or anything that reads as transactional in the first touch. Reframe as "how we could put <Company>'s story in front of the buyers you're trying to reach" or "happy to share a few ideas for what's worked for partners in your space." **Calendly close when Kerri is the sender:** `"Here's Brian's calendar link."` — Kerri routes the meeting to Brian explicitly, so the recipient knows who they'll be meeting with. Links to `https://calendly.com/brian-hardwarefyi/30min`. (When Brian is the sender he uses "My calendar can be found here.") Structure (cold first-touch, updated 2026-06-19 — open on their world, playbook A1): personalization hook (their world, the FIRST line) → role intro + audience proof in one line (AFTER the hook, never before it) → value-framed fit → one low-friction interest/value ask (never a meeting time on a cold first touch) → sign-off (`Kerri` by default, `Brian` only when explicitly his thread). The old identity-first order (role intro before the hook) is RETIRED for cold first-touches; the `kerri-cold-outreach` STEP 4 four-beat is canonical. (Warm/re-opening threads where we already have a relationship can still lead with the warmer "Hi X" beat.)
