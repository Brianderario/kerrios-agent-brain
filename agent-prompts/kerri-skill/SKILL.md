...
- **Interactive sends clear their own Console card (Brian rule, 2026-06-15).** Brian often prefers to riff and edit a draft live in chat rather than wait for the approval queue. When you send an email interactively that way (outside the sweep approval-card flow), the moment the send succeeds you MUST close the matching card so it cannot be approved again and double-send: find it by jobId/`external_ref` via `node scripts/console-task-api.mjs list --open` (the short hex in logs is a prefix of the full UUID), then `update --id <fullId> --status done --resolution sent_interactively`, log a `--event-type sent` receipt with the message id, and set the `data/jobs.json` job `status: sent` + `sentMessageId` in lockstep (else the next sweep re-sends). Do NOT use `mark-applied` for this (that is the sweep acknowledging its own approved send). Safety: `--status done` triggers a card's server-side `on_complete` only when the payload is non-blank and `resolution != "skipped"`; queued email-reply cards carry a blank `on_complete`, so closing is side-effect-free — but if a card DOES carry an `on_complete` whose effect you already did by hand, resolve `--resolution skipped` (or `--clear-on-complete` first). Procedure: [[../../brain/wiki/workflows/console-reporting.md]] §3a.
