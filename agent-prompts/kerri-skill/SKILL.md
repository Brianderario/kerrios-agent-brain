...
- **Definition-of-Done gate before any composed send (Brian rule, 2026-06-17).** Run the 7-step pre-send gate in `[[../../brain/wiki/workflows/definition-of-done-gate.md]]`: read the full state (thread + brain log), name the real deliverable (not the nearest reply), research+inventory, attach what we hold, completeness test, escalate-not-park, restate before send. The next several bullets (brain-log check, no double emails) are components of this one gate; run them as a single checklist, never piecemeal. From the 2026-06-17 retro: ~25% of net-new asks opened with a correction, nearly all the same root cause (holding output instead of finished deliverable, acting before reading existing state).
- **Brain-log check before any composed send (Brian rule, 2026-06-11) [gate step 1].** Unless Brian directly dictated the exact content, check `NOW.md` and grep `brain/log.md` for the topic before sending any email, so the message reflects work already completed or in flight. That is what the log is there for. Origin: the 6/11 DTW miss — the listing was published and logged at 00:05 ET, yet a 02:38 sweep reply told Benji it was still queued.
...
- **ALL drafts go to Kerri Console tasks** - create them with `node scripts/console-task-api.mjs create`, status `needs_approval`, deterministic `external_ref`, and the canonical block format with ACTION line + CONTEXT + DRAFT (see [[../../kerri-inbox-sweep/SKILL.md]] "CREATE THE CONSOLE TASK" section for the exact template). Every reply must also pass `--reply-to-message-id`, `--reply-to-mailbox`, and `--reply-to-conversation-id` from the live thread. Missing route metadata is a hard create failure, never a reason to remove `Re:` or send a fresh chain. Savant's deterministic sender owns all approved sends, including isolated multi-draft batches; the inbox sweep only reconciles message-level proof. This applies to inbox-sweep drafts and ad-hoc drafts. Slack/iMessage are notification and status channels, never the approval surface.
- **Every applied Console decision needs a receipt.** Savant's deterministic sender writes `applied_at`, provider message proof, and the sent event itself for approved individual and batch email; agents must not duplicate or overwrite that proof. After an agent acts on a skipped, redo, suggestion, or interactive decision, call `node scripts/console-task-api.mjs mark-applied --id <taskId> --note "<result>"`. If you have concrete proof such as a message id, draft id, connector, or sent timestamp, also call `node scripts/console-task-api.mjs event --id <taskId> --event-type sent --note "<what happened>" --metadata-json '{"message_id":"..."}'`. Brian must be able to open the task and see what happened.
- **Interactive sends clear their own Console card (Brian rule, 2026-06-15; hardened 2026-06-17).** Brian often prefers to riff and edit a draft live in chat rather than wait for the approval queue. When you send an email interactively that way (outside the approval-card sender flow), the moment the send succeeds you MUST close the matching card so it cannot be approved again and double-send. **Do it with the one atomic command, never the steps by hand:** `node scripts/console-task-api.mjs close-job --job <jobId> --message-id <id> --note "<what/where>"`. That single call closes the send card (`done`/`sent_interactively`), closes the paired recap card (from the job's `routing.recapCardId`), logs the `sent` receipt, verifies the card reads `done`, then flips the `data/jobs.json` job (`status`, `resolution`, `sentAt`, `sentMessageId`) - card-first so a partial failure can never leave a re-approvable card behind. It is idempotent and auto-selects the open job for that jobId; pass `--card <consoleTaskId>` if duplicates are ambiguous, `--dry-run` to preview. Do NOT use `mark-applied` for this because `close-job` writes the complete proof atomically. Safety is built in: `--status done` fires a card's server-side `on_complete` only when the payload is non-blank and `resolution != "skipped"`; queued email-reply cards carry a blank `on_complete`, so closing is side-effect-free. Procedure: [[../../brain/wiki/workflows/console-reporting.md]] §3a.
