...
|---|---|---|---|
| `kerri-inbox-sweep` | `*/15 6-23 * * 1-5` | `kerri-inbox-sweep/SKILL.md` | 4 mailboxes → drafts/tasks → Kerri Console approval → send on approval → `jobs.json` + cursors + grades + `draft-learnings.md` → `💡 SUGGESTION` task. Weekday every-15-min coverage only (06:00–22:45 ET); weekends handled by `kerri-inbox-sweep-weekend`. |
...
| Revenue standup | `0 16 * * 5` | `kerri-revenue-standup/SKILL.md` | **LIVE as an Anthropic cloud routine** (trig_01FPKyYXmY4goNNHdkdXLLSz), Fridays ~4pm ET, since 2026-06-11; the local Mac scheduled task was disabled the same day (re-enable only if the cloud routine is removed). Read-only scoreboard vs $1M, pipeline velocity, outreach conversion, renewal radar, top 3 actions → Slack DM (no text; Kerri stopped texting 2026-06-17). Reports via Slack, not create_agent_run, so it carries no Console `report_interval_hours` and is intentionally outside Console + local liveness checks. State: `data/revenue-standup-state.json`. |
