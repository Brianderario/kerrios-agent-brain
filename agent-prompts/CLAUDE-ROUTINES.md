...
| Revenue standup | `0 16 * * 5` | `kerri-revenue-standup/SKILL.md` | **MIGRATED TO CLOUD 2026-06-11.** The local `kerri-revenue-standup` scheduled task is `enabled:false` (RETIRED), replaced by the cloud routine `kerri-revenue-standup` (`trig_01FPKyYXmY4goNNHdkdXLLSz`); re-enable the local task only if the cloud routine is removed. Read-only scoreboard vs $1M, pipeline velocity, outreach conversion, renewal radar, top 3 actions → Slack DM + text. State bootstrapped at `data/revenue-standup-state.json`. |
...
| Renewal watchdog | `0 10 * * 3` | `kerri-renewal-watchdog/SKILL.md` | **LIVE — Claude Code scheduled task** (Wednesday ~10am ET, `enabled:true`, firing on cadence; last fire 2026-06-10, next 2026-06-17 as of this reconcile). Scans CRM for expiring contracts, lapsed sponsors, upsell opportunities; max 5 renewal drafts per run, approval-gated. State at `data/renewal-watchdog-state.json`. |
