...
|---|---|---|---|
| `kerri-inbox-sweep` | `0,30 6-23 * * 1-5` | `kerri-inbox-sweep/SKILL.md` | 4 mailboxes → drafts/tasks with exact reply routes → Kerri Console approval → Savant deterministic individual or isolated batch send → sweep proof reconciliation plus skip/redo → `jobs.json` + cursors + grades + `draft-learnings.md` → `💡 SUGGESTION` task. Weekday every-30-min coverage (:00/:30, 06:00–23:30 ET); weekends handled by `kerri-inbox-sweep-weekend`. |
| `kerri-inbox-sweep-weekend` | `0 18 * * 6,0` | `kerri-inbox-sweep/SKILL.md` (weekend single-run variant) | Same routine as `kerri-inbox-sweep`, one run at 18:00 ET Sat + Sun. Shares `inbox-sweep-state.json`; monitored:false in the manifest because the `kerri-inbox-sweep` evaluator's weekend branch covers its liveness off that shared state file. |
...
| `kerri-build-proposal` | `0 23 * * *` | `kerri-build-proposal/SKILL.md` | vision doc + ledgers + live revenue state → single most valuable overnight slice (revenue first) → one internal email to Brian with the exact build-loop prompt; Brian's "go" launches it, the routine never does → `brain/log.md` line + heartbeat. Added 2026-06-12 (Brian standing instruction, iMessage). |
...
| Revenue standup | `0 16 * * 5` | `kerri-revenue-standup/SKILL.md` | **LIVE local Claude Code scheduled task** (Fridays ~4pm ET); re-enabled 2026-06-23 off the retired Slack cloud routine. Read-only scoreboard vs $1M, pipeline velocity, outreach conversion, renewal radar, top 3 actions → filed as a **Console task card** (STEP 6), never Slack/text. Reports the run into the Console via STEP 8 heartbeat (`create_agent_run`), so `report_interval_hours: 192` is valid and the Console health-check sees runs. State: `data/revenue-standup-state.json`. |
...
| Pipeline staleness | `50 8 * * 1-5` | `kerri-pipeline-staleness/SKILL.md` | **Claude Code scheduled task** (weekday ~8:50am ET). Enforces the no-reply decay rule (Brian 2026-06-14): 30d no prospect reply → demote one phase (act-and-report); 60d → approval-gated `closed_lost` suggestion (never auto-closes). Keyed on the prospect's real last reply read from the mailboxes, not CRM `last_activity_at`. Decision logic is the tested `scripts/pipeline-staleness.mjs` (`--max-demote` fail-closed cap). State: `data/pipeline-staleness-{signals,report}.json`. |

**Signed → invoiced (folded into the inbox sweep, NOT a separate routine; Brian 2026-06-22):** when the inbox sweep transitions a deal to `closed_won` off a signed-contract email, the same run stages the Mercury invoice as a 🧾 INVOICE one-click approval card and attaches the executed contract. Spec: `agent-prompts/kerri-inbox-sweep/references/invoice-on-win.md`; trigger lives in the sweep's CRM PIPELINE AUTO-UPDATE step. Never auto-sends, never double-invoices. (An earlier standalone kerri-auto-invoice scheduled task was set up then retired the same day in favor of this in-sweep hook; the disabled MCP entry lingers only because the scheduled-tasks MCP has no delete API — it has no files and cannot run.)
| Renewal watchdog | `0 10 * * 3` | `kerri-renewal-watchdog/SKILL.md` | **LIVE Claude Code scheduled task** (Wednesdays ~10am ET; last fired 2026-06-17). Scans CRM for expiring contracts, lapsed sponsors, upsell opportunities; max 5 renewal drafts per run, approval-gated. State: `data/renewal-watchdog-state.json`. |
