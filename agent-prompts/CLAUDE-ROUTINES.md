...
| Renewal watchdog | `0 10 * * 3` | `kerri-renewal-watchdog/SKILL.md` | **Prompt ready; shim prepared 2026-06-09; schedule activation pending Brian confirmation.** (The scheduler registration already exists and is enabled as of the 2026-06-09 migration; first fire would be Wednesday ~10am ET. Brian has not yet confirmed activation.) Scans CRM for expiring contracts, lapsed sponsors, upsell opportunities; max 5 renewal drafts per run, approval-gated. State bootstrapped at `data/renewal-watchdog-state.json`. |
