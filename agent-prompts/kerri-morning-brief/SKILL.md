...
- Get goal-progress numbers from the Savant deals API, the system of record. **The old `GET /api/v1/revenue_command` roll-up endpoint has been retired and now returns 404 — do not call it.** Read `GET /api/v1/deals` instead: each deal carries `value`, `weighted_value`, `stage`, `renewal_status`, and `fulfillment_summary` (the owed-deliverables detail). Booked = sum of `value` for won deals; open = sum of `value` across open Prospect/Interest deals; weighted = sum of `weighted_value`; owed-deliverables = roll up each open/won deal's `fulfillment_summary` — all against the $1M target. For the canonical topline already bucketed by status (Prospect / Interest / Contract Won / Contract Lost), use the scoreboard helper `node scripts/hwfyi-revenue-goal-sheet.mjs --pipeline-summary` (returns `booked/open/weighted/byStatus`), the same scoreboard the Friday revenue standup uses; if Sheets is unavailable, fall back to aggregating `GET /api/v1/deals` directly. The `CY2026 Revenue Goal` Sheet tab is a one-way mirror for cross-checking only; if it disagrees with the Savant deals, Savant wins.
