...
**Interactive email review:** Savant Tasks remains the default for unattended and agent-proposed external mail. The intended exchange with Codex, Kerri in Slack, or Muse is: show the exact proposed email, hear Brian say "send it," dispatch it once through a guarded path, verify provider/Sent Items evidence, and continue the conversation. His authenticated chat decision is the single approval only where actor-and-payload binding is implemented; an internal canonical card must not demand a second user-facing decision. Report a queued or uncertain send honestly and reconcile it without blind retry. A changed draft or second send needs fresh approval. Standard & Works content never enters KMG Savant state. Muse's current custom API-key path still requires its Microsoft-backed approval page until the broker can independently verify a chat decision.
