...
- Authorized agents may work on Standard & Works. File its Savant records under property slug `standard-and-works` and enforce brand access so Benji cannot see its internal operations, finances, compensation, or content drafts. Never leak sensitive detail through broadly visible cards, notifications, prompts, or untagged brain records; if isolation cannot be enforced, keep it in the property-scoped source and report the gap.
